namespace echo17.EndlessBook
{
    ...
    public class EndlessBook : MonoBehaviour
    {
        ...
        {
            // 1. Затемняем экран, как и раньше
            yield return ScreenFaderTransition.Instance.FadeToBlack(null);

            // 2. СНАЧАЛА отправляем событие, чтобы GameScenario успел включить основную сцену
            _eventManager.ReportMiniGameCompleted();

            // 3. Ждем один кадр, чтобы гарантировать, что EventBus обработал событие
            yield return null;

            // 4. ТЕПЕРЬ выгружаем сцену мини-игры. Камера из MainGame уже должна быть активна.
            AsyncOperation asyncUnload = SceneManager.UnloadSceneAsync("Diary");
            yield return asyncUnload;
        }

        protected virtual void Initialize()
        {
            if (ScreenFaderTransition.Instance != null)
            {
